...
{
    public partial class Search : ArticleModuleBase
    {
        ...
        public void BindAuthorList()
        {
            ...
            List<AuthorInfo> listAuthor = AuthorController.GetByType(Settings_Search.General.TypeId);
        ...
        {
            List<CategoryInfo> list = CategoryController.GetModuleCategories(Settings_Search.General.CategoryList);
            foreach (var item in list)
            {
                LocalizeObject_Category(item);
            }
        ...
        {
            int tab = GetSubModulePageId_List(Settings_Search.General.ArticleListPage, Settings_Search.General.TypeId);

            return Globals.NavigateURL(tab);
        }
        protected void btnAdvancedSearch_Click(object sender, EventArgs e)
        {
            Cross.DNN.Common.RemoteForm.RemoteForm remoteForm = new DNN.Common.RemoteForm.RemoteForm("ArticleSearch", GetUrl());

            remoteForm.Fields.Add("ArticleTypeId", Settings_Search.General.TypeId.ToString());
            ...
            if (Settings_Search.Advanced.BuiltInField_EventStart
           && ddpEventStart.SelectedDate != null
           && !string.IsNullOrEmpty(ddpEventStart.SelectedDate.Value.ToString()))
            {
                if (LocalUtils.IsDate(ddpEventStart.SelectedDate.Value.ToString()))
                {
                    searchDate = ddpEventStart.SelectedDate.Value;
                    remoteForm.Fields.Add("S_EventStart", searchDate.ToShortDateString());
                    remoteForm.Fields.Add("S_EventStart_Oper", ddlOper_EventStart.SelectedValue);
                }
            }

            if (Settings_Search.Advanced.BuiltInField_EventEnd
           && ddpEventEnd.SelectedDate != null
           && !string.IsNullOrEmpty(ddpEventEnd.SelectedDate.Value.ToString()))
            {
                if (LocalUtils.IsDate(ddpEventEnd.SelectedDate.Value.ToString()))
                {
                    searchDate = ddpEventEnd.SelectedDate.Value;
                    remoteForm.Fields.Add("S_EventEnd", searchDate.ToShortDateString());
                    remoteForm.Fields.Add("S_EventEnd_Oper", ddlOper_EventEnd.SelectedValue);
                }
            }
        ...
        protected void btnSearch_Click(object sender, EventArgs e)
        {
            ...
            parm = "&ArticleTypeId=" + Settings_Search.General.TypeId.ToString();
            if (Settings_Search.Basic.ShowCategory)
            {
                parm += "&ArticleCategoryId=" + ddlCategory.SelectedValue;
            }
            if (Settings_Search.Basic.ShowAuthorList)
            {
                parm += "&ArticleUserId=" + ddlAuthorList.SelectedValue;
            }
            ...
            if (!string.IsNullOrEmpty(txtSearch.Text.Trim()))
            {
                ...
                if (s != "")
                {
                    ...
                    {
                        parm += "&ArticleSearchTerm=" + Server.UrlEncode(s);
                    }
                    else
                    {
                        parm += "ArticleSearchTerm=" + Server.UrlEncode(s);
