<%@ Control Language="C#" AutoEventWireup="true" CodeFile="TagList.ascx.cs" Inherits="Cross.Modules.Article.View.TagList" %>
<style type="text/css">
    a.Tag1
    {
        font-size: 0.7em;
...
<asp:DataList ID="dlTag" runat="server" RepeatColumns="<%#Settings_TagList.General.ColCount %>">
    <ItemTemplate>
        <table width="100%" border="0" cellpadding="0" cellspacing="1">
            <tr align="left">
                <td>
                    <%if (Settings_TagList.General.DisplayArticleCounts)
                      { %>
                    <asp:HyperLink CssClass='<%#Convert.ToString(Eval("CloudCssLevel")) %>' ID="lnkName"
                        runat="server" Text='<%# Convert.ToString(Eval("Tag")) +"(" +Convert.ToString(Eval("ArticleCounts"))+")" %>'
