...
namespace Cross.Modules.Article.View
{
    ...
    /// </history>
    /// -----------------------------------------------------------------------------
    partial class TagDashboard : ArticleModuleBase
    {
        char seprator = '|';//分隔符    
        ...
        private void BuildTagScript()
        {
            ...
            string flashFile = DotNetNuke.Common.Globals.ApplicationPath + "/DesktopModules/CrossArticle/Flash/TagCloud.swf";

            int tagLimit = list.Count > Settings_TagDashboard.General.Rows ? Settings_TagDashboard.General.Rows : list.Count;

            var tagList = (from p in list orderby p.ArticleCounts descending select p).Take(tagLimit);
        ...
        protected string GetNavigate(int tagId)
        {
            ...
            return Globals.NavigateURL(tab, "", "ArticleTagId=" + tagId.ToString() + "&ArticleTypeId=" + Settings_TagDashboard.General.TypeId.ToString());
        ...
        private void RegisterSwfObject()
        {
            if (!this.Page.ClientScript.IsClientScriptBlockRegistered("UltraFlashSwfObject"))
            {
                ...
                sb.AppendFormat("<script type=\"text/javascript\" src=\"{0}/Js/swfobject.js\"></script>{1}",
                    DotNetNuke.Common.Globals.ApplicationPath + "/DesktopModules/CrossArticle", "\r\n");
