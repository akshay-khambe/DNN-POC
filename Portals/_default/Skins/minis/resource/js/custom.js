(function ($) {
    ...
    image.onerror = function () {
        if (typeof GetWebpConvertServiceUrl != "undefined" && $(".d-wrapper").eq(0).data("mid")) {
            let WebpServiceUrl = GetWebpConvertServiceUrl + $(".d-wrapper").eq(0).data("mid") + "&f=png&WebP=";
            let LocalUrl = window.location.host;
            $('.content-builder img[src*=".webp"],.content-builder img[data-src*=".webp"],.content-builder [style*=".webp"],.content-builder a[href*=".webp"],.content-builder video[poster*=".webp"]').each(function () {
				...
				if(isImg == "IMG"){
					...
					url = $(this).attr("poster");
                }else{
					url = this.style.backgroundImage.split('"')[1];
				}

                if(url.indexOf(LocalUrl) != -1){url =url.split(LocalUrl)[1];};
...
if (jQuery(window).width() < 992) {
	if (jQuery(".mobile-header").css("position") == "fixed") {
		floatingOffset = $(".mobile-header-replace").height() || $(".mobile-header").height();
	} else {
		floatingOffset = 0;
	}
...
jQuery(window).resize(function() {
	if (jQuery(window).width() < 992) {
		if (jQuery(".mobile-header").css("position") == "fixed") {
			floatingOffset = jQuery(".mobile-header-replace").height() || $(".mobile-header").height();
		} else {
			floatingOffset = 0;
		}
	} else {
...
(function($) {
	...
	// Get Url Param
	//----------------------------------------------------/
	DNG.GetUrlParam=function(paraName){var url=document.location.toString();var arrObj=url.split("?");if(arrObj.length>1){var arrPara=arrObj[1].split("&");var arr;for(var i=0;i<arrPara.length;i++){arr=arrPara[i].split("=");if(arr!=null&&arr[0]==paraName){return arr[1]}}return""}else{return""}};
	//----------------------------------------------------/
	// Fixed Footer
	//----------------------------------------------------/
					...
					if ($(window).scrollTop() >= top) {
						if (!e.hasClass("floating-active")) {
							if (e.hasClass("floating-nav-fade")) {
								e.stop().css("opacity", "0").animate({
									"opacity": "1",
								}, 500).addClass("floating-active");
								...
								e.stop().css({
									...
									"opacity": "1",
									top: "0"
								}, 300).addClass("floating-active");
							}else if (e.hasClass("floating-nav-sticky")) {
								e.addClass("floating-active");
							}
						} 
					} else {
				...
				if (e.hasClass("floating-nav-scrollup")){
					...
					$(window).scroll(function() {
						if($(window).scrollTop() > scroll){
							...
							if(e.hasClass("floating-active-down") &&  $(window).scrollTop() >= top  ){						
								e.removeClass("floating-active-down").addClass("floating-active-up");
							}
							//console.log(scroll)
						}else{
							if ($(window).scrollTop() <= top2) {
								e.removeClass("floating-active floating-active-up floating-active-down");
								scroll =top;
								return;
							}else if(scroll-$(window).scrollTop() > 10 && $(window).scrollTop() >= top){ 
								scroll =$(window).scrollTop();
							//	console.log(scroll)			 
								e.removeClass("floating-active-up").addClass("floating-active-down");
							}	
	...
	DNG.backToTop = function() {
		...
		var shareout="";
		var length = share.prevAll().length;
			share.on("mouseover", function() { 
					if($(window).width()<=767) return false;
					$(this).addClass("active").prevAll().each(function(i){
						$(this).stop().delay(40*i).queue(function(){$(this).addClass("active");$(this).dequeue();})
					})
				}).on("click", function() { 
					if(share.hasClass("active")){
						share.removeClass("active").prevAll().each(function(i){
							$(this).stop().delay(40*(length-i)).queue(function(){$(this).removeClass("active");$(this).dequeue();})
						})
					}else{
						$(this).addClass("active").prevAll().each(function(i){
							$(this).stop().delay(40*i).queue(function(){$(this).addClass("active");$(this).dequeue();})
	...
	DNG.mobileheadernavbar = function() { /*mobile header navbar*/
		$("body > form").append($(".mobile-header-navbar .mega-menu-wrap").addClass("mobile_menu").append("<span class='mobile_menu_close lnr lnr-cross'></span>"));

		if ($("#wpadminbar").length != 0 && $(".mobile-header").css("position") == "fixed") {
			$(window).scroll(function() {
				$(".mobile-header").css("top", Math.max($("#wpadminbar").height() - $(window).scrollTop(), 0));
			})
		}
	...
	DNG.pageTitle = function() {
		$("#page-title").each(function() {
			...
			if (e.data("height") === "full") {
				e.children(".page-title-container").innerHeight($(window).height() - e.offset().top);
				$(window).resize(function() {
					e.children(".page-title-container").innerHeight($(window).height() - e.offset().top);
				});
			}
			e.find(".page-title-arrow").on("click", function() {
				jQuery('body,html').stop().animate({
					scrollTop: e.innerHeight() + e.offset().top - floatingOffset
				}, ($("html").css("scroll-behavior")=="smooth"?0:800));
			});
	...
	DNG.sidebarSticky = function() {
		...
		$(".sidebar-fix").each(function(){
			var box = $(this);
			var container = box.find(".container.has-sidebar");
			var left = box.data("width");

			if(!container.length) return ;
			function sidebarSwitch(){
				if(container.offset().left < left){
					$("html").addClass("switch-sidebar");
				}else{
					$("html").removeClass("switch-sidebar");
				}
			}	
			sidebarSwitch();
		...
		//	if ($(this).data("sticky") == "on") {
		//		return;
		//	}

			if( typeof DNNapplyTypography != "undefined" ){ return;}
			
			var wpadminbar = $("#wpadminbar").length == 0 ? 0 : $("#wpadminbar").height();


			var e = $(this),
			//	parent = e.parent(".sidebar_dynamic,.sticky_media_parent"),
			//	parentbox = e.parents("[data-sticky=parent]"),
				parent = e.parent(),
	...
	DNG.anchorSmooth = function() {
		...
		if(OnePageOption.duration){
			...
			$(".primary_structure >li.current >a,.menu_list.mm-listview > li.current >a,.mobilemenu-main .gomenu>ul>li>a").each(function(){
				var h =$(this).attr("href");
				if(h == window.location.href.split("#")[0] || h.substring(0,h.length-1) == window.location.href.split("#")[0]){
					$(this).on("click",function(e){
						e.preventDefault();
						if($(this).parents(".mm-listview").length){$(".dng-mobilemenu .mobile-menu-icon").eq(0).click();}
						scrollbox.stop().animate({scrollTop: 0},($("html").css("scroll-behavior")=="smooth"?0:(OnePageOption.duration?OnePageOption.duration:500)),OnePageOption.easing);
						window.history.pushState({},0,h);
						return false;
					}).addClass("dng-anchor").attr("href","#Body")
				}
			})
		...
		$('.content-builder,.header_bg,.mobile-header,.dng-leftmenu,.dng-megamenu,.dng-sidepanemenu,#anchorNav,.dng-mobilemenu,.mobilemenu-main .gomenu').find('a[href*="#"]:not(.mm-btn):not(.mm-next):not(.mm-title)').each(function(){ 
			if(!$(this).hasClass("LightBox") && !$(this).hasClass("is-lightbox")){
				...
				if(! reg.test(this.hash.trim().substr(1))) {
					...
					if ($target.length && url == urlhref ) {
						$(this).addClass('dng-anchor').on("click",function(e){ 
							e.preventDefault();
							...
							},($("html").css("scroll-behavior")=="smooth"?0:(OnePageOption.duration?OnePageOption.duration:500)),OnePageOption.easing?OnePageOption.easing:"swing");

							if($hash!="#Body"){window.history.pushState(null, null, $hash);}
						});	
					}
				}
			}
		...
		function menuAnchorPosition(menu){
			...
			if(parseInt(w.scrollTop()) +  w.height() ==  parseInt($(document).height())){
				...
				$current.parent().addClass("current").children("a").focus();
	...
	DNG.editMode_panename = function() {
		$(".addModuleHandler").each(function(){
			$(this).parent().attr("id") && $(this).after("<div class=\"edit-panename\">"+$(this).parent().attr("id").split("dnn_")[1]+"</div>")
		})
	};
	...
	// Edit carousel04
	//----------------------------------------------------/
	DNG.carousel04 = function() {
		var buttonBox = $(".carousel04-buttonBox");
        if( $(window)[0].innerWidth < 768 ){
            buttonBox.height(buttonBox.find(".box").height());
        }else{
	...
	DNG.dngparallax = function() {
		...
		$('[data-parallax="youtube"],[data-parallax="video"]').each(function(){
			var e = $(this);
			e.prepend('<div class="dng-video-wrapper"><div class="dng-video-responsive"></div></div>');
			...
			if(e.data("parallax")=="youtube"){
				...
				responsive.append('<div class="playerBox youtube-playerBox" id="'+id+'" style="width:300px;height:150px;"></div>')
				...
				window.onYouTubePlayerAPIReady = function() {
					...
					function onPlayerReady(event){ 
						...
						if(e.data("viewport")){
							if(!play && $(window).scrollTop() + parseInt($(window).height()) > offsetTop  && $(window).scrollTop()  < offsetTop + boxheight ){
								event.target.playVideo();
								play =true;
							}else{
							...
							$(window).scroll(function(){
								...
								if(play && ($(window).scrollTop()  > offsetTop + boxheight ||  $(window).scrollTop() + parseInt($(window).height()) < offsetTop )){
									event.target.pauseVideo();
									play =false;
								}
							})
				...
				if(e.data("webm")) html+=' <source src="'+e.data("webm")+'" type="video/webm">';
				html+='</video>';
			...
			var width =	(parseInt(e.innerHeight())/parseInt(box.innerHeight())) * (parseInt(box.innerWidth())/parseInt(e.innerWidth())) *  parseInt(e.innerWidth()) ;
				box.innerWidth(width+2);
				$(window).resize(function(){
					width = (parseInt(e.innerHeight())/parseInt(box.innerHeight())) * (parseInt(box.innerWidth())/parseInt(e.innerWidth())) *  parseInt(e.innerWidth()) ;
					box.innerWidth(width+2);
	...
	$(document).ready(function() {
		$(window).on("load", function() { 
			if($("body").hasClass("body-side-static-left") || $("body").hasClass("body-side-static-right")){
			//	setTimeout(function(){
					DNG.pageLoaders();
			//	},3000);
			}else{
			...
			if(typeof Main != "undefined" && typeof Main.LightBoxPatch != "undefined") Main.LightBoxPatch();
		});
...
$(document).ready(function (e) {
	...
	//	if ($(this).parents(".is-section").nextAll('div:not(.is-hidden)').html()) { /* .hidden class can be used as an exception */
            $('html,body').animate({
                scrollTop: $(this).parents(".is-section").offset().top + parseInt($(this).parents(".is-section").height()) 
            }, ($("html").css("scroll-behavior")=="smooth"?0:800));
    //    }
        e.preventDefault();
...
$(document).ready(function() {
	...
	function tabs(){
		...
		var box=$(this).parents(".section-tabs-list,.section-tabs-indexed").siblings(".section-tabs-container").find(".is-container").eq(index);
			box.fadeIn().addClass("active").siblings(".is-container").hide().removeClass("active");
		$(window).scroll();
		if($(window).width()<768){
			jQuery('body,html').stop().animate({
				scrollTop: box.offset().top -floatingOffset-30
			}, ($("html").css("scroll-behavior")=="smooth"?0:800));
		}
	}
...
$(document).ready(function() {
	...
	var ftop = $(".mobile-header").hasClass("header-position")? 1: parseInt($(".mobile-header").height());

	$(window).scroll(function() {
		if($(".mobile-header").hasClass("floating-nav")){
			if($(window).scrollTop() > ftop){
				$(".mobile-header").addClass("floating-nav-active")
			}else{
				$(".mobile-header").removeClass("floating-nav-active")
			}
		}
	})
...
$(function () {
    if ($(".header-replace.header-position").length && $(".ContentPane").is(":visible") && !$(".ContentPane").find(".content-builder").length && $(".ContentPane").offset().top <= 0) {
        $(".ContentPane").css("padding-top", $(".header-replace.header-position").height() + 30);
    }
})
...
$(function () {
	$(".languageBox").each(function () {
		...
		e.find(".language-current").on("click", function (event) {
			...
			e.find(".language-list").stop().fadeToggle();
		...
		$(document).on("click", function (event) {
			if (!e.is(event.target) && e.has(event.target).length === 0) {
				...
				e.find(".language-list").fadeOut(function(){e.find(".language-current").attr("aria-expanded","false");});
			}
		})
