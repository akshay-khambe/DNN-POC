...
<script type="text/javascript">
    ...
    jQuery(function ($) {
       ...
       <% } %>
        ...
        <% if(IncludeDatePicker == true) { %>
            var Async_datepick<%=ModuleId %> = setInterval(function(){
                if("function" == typeof $(document).datepick){
                    $(".form_div_<%=ModuleId %> .jquery-datepick").datepick({ dateFormat: '<%=DatePattern %>',onSelect:function(e){
                       $(this).trigger("focus").trigger("blur");
                    } });
         			clearInterval(Async_datepick<%=ModuleId %>);
                    }
                },100)
        <% } %>
...
<% if (Settings_Recaptcha_Enable){%>
    <% if (ViewSettingT("PowerForms_Recaptcha_Type", 0) == 2){%>
    var GRecaptchaVerifyCallback<%=ModuleId %> = function (token) {
        $("#GoogleRecaptchaResponse<%=ModuleId %>").val(token);
    ...
    jQuery(function ($) {
        var Async_grecaptcha<%=ModuleId %> = setInterval(function () {
                if ("undefined" != typeof grecaptcha) {
                    ...
                    clearInterval(Async_grecaptcha<%=ModuleId %>);
                }
            }, 100)
        });
    <%}%>
