...
namespace Cross.Modules.Article.View
{
    ...
    /// </history>
    /// -----------------------------------------------------------------------------
    partial class ListDashboard : ArticleModuleBase
    {
        char seprator = '|';//分隔符    
        ...
        /// <summary>
        /// 根据设置截断太长的article title
        /// </summary>
        /// <param name="title"></param>
        /// <returns></returns>
        protected string GetShortenTitle(string title)
        {
            return Settings_ListDashboard.General.TitleLength > 0 ?
                DotNetNuke.Common.Utilities.HtmlUtils.Shorten(title, Settings_ListDashboard.General.TitleLength, "..") : title;
        }
        private void BuildNavigateScript()
        {
            List<ArticleInfo> list = ArticleController.GetFlashImage(Settings_ListDashboard.General.TypeId, GetUserId(), Settings_ListDashboard.General.CategoryList,
                Settings_ListDashboard.General.PlayMode == LocalUtils.Flash_PlayMode_ImageNavigate ? 9 : Settings_ListDashboard.General.Rows,
                Settings_ListDashboard.General.ShowFeaturedOnly,
                Settings_ListDashboard.General.IgnoreSortIndex,
            ...
            if (Settings_ListDashboard.General.PlayMode == LocalUtils.Flash_PlayMode_ImageNavigate_ButtonFree)//button free flash
            {
                if (Settings_ListDashboard.NavigateButtonFree.NavigateButtonFree_Show_Text)
                {
                    flashHeight += 20;
                }
            }
            ...
            switch (Settings_ListDashboard.General.PlayMode)
            {
                case LocalUtils.Flash_PlayMode_ImageNavigate:
                    flashFile = DotNetNuke.Common.Globals.ApplicationPath + "/DesktopModules/CrossArticle/Flash/ImageNavigate_Classical.swf";

                    break;
                case LocalUtils.Flash_PlayMode_ImageNavigate_Integrative:
                    flashFile = DotNetNuke.Common.Globals.ApplicationPath + "/DesktopModules/CrossArticle/Flash/ImageNavigate_Integrative.swf";
                    break;
                case LocalUtils.Flash_PlayMode_ImageNavigate_ButtonFree:
                    flashFile = DotNetNuke.Common.Globals.ApplicationPath + "/DesktopModules/CrossArticle/Flash/ImageNavigate_ButtonFree.swf";
                    break;
            ...
            for (int i = 0; i < maxItem; i++)
            {
                ...
                string pic = list[i].Image;
                if (string.IsNullOrEmpty(list[i].Image))//image 为空，试用缺省图片
                {
                    pic = LocalUtils.DefaultPicture;
                }
                else
                {
                    if (pic.StartsWith("FileID="))//以FileID开头，获取其地址
                    {
                        pic = GetRelativeUrlByFileId(list[i].Image);//如果该产品图片在后台被删除，则用缺省图片
                        if (pic == "")
                        {
                            pic = LocalUtils.DefaultPicture;
                        }
                ...
                pics += pic;
                links += GetNavigate(list[i].Id, list[i].Title);
                texts += GetShortenTitle(list[i].Title);
            }
            //替换掉"&" 为 "%26",防止链接出错
...
//一点点去掉代码发现是 <param name="scale" value="exactfit"> 这个参数作怪。在 Flash 的脚本里面强制指定了 Stage.scaleMode = "noScale"; 
        ...
        private string GetNavigate(int articleId, string title)
        {
        ...
        private void RegisterSwfObject()
        {
            if (!this.Page.ClientScript.IsClientScriptBlockRegistered("UltraFlashSwfObject"))
            {
                ...
                sb.AppendFormat("<script type=\"text/javascript\" src=\"{0}/Js/swfobject.js\"></script>{1}", DotNetNuke.Common.Globals.ApplicationPath + "/DesktopModules/CrossArticle", "\r\n");
